...
class TransaksiController extends Controller
{
    ...
    {
//        $data = DB::table('tb_transaksi')->where('id_transaksi', \DB::raw("(select max(`id_transaksi`) from tb_transaksi)"))->first();

        $data = DB::table('tb_detail_transaksi')->where('transaksi_id',$id)
            ->join('tb_transaksi', function($join){
                $join->on('tb_detail_transaksi.transaksi_id','=','tb_transaksi.id_transaksi');
            })
            ->join('tb_paket', function($join){
